...
class ImageViewWidget(QWidget):
    ...
    def update_info(self):
        ...
        n_fr = self.__reader.frame_count
        fps = self.__reader.frame_rate

        self.__label_info.setText(
            f'{int(ts) // 60:3d}:{int(ts) % 60:02d}.{(ts - int(ts)) * 1000:03.0f} ({idx:7d}/{n_fr:7d}) fps={fps:.2f}'
        )
    ...
    def on_key_press(self, key):
        ...
        if key == Qt.Key_Q:
            ...
            self.set_frame(delta=-int(self.__reader.frame_rate / 5))
        elif key == Qt.Key_D:
            self.set_frame(delta=+int(self.__reader.frame_rate / 5))
        elif key == Qt.Key_Z:
            self.set_frame(delta=-int(self.__reader.frame_rate * 10))
        elif key == Qt.Key_C:
            self.set_frame(delta=+int(self.__reader.frame_rate * 10))
...
class MarkerWidget(QWidget):
    ...
    def init_ui(self):
        ...
        widget_wd = DirectoryChooserWidget('Working Directory',
                                           os.path.expanduser(r'~/Desktop/VideoMarkerData'))
        ...
        combo_sl = QComboBox(self)
        combo_sl.currentIndexChanged.connect(self.combo_sl_changed)
        layout_sl.addWidget(combo_sl)
        self.__combo_sl = combo_sl
    ...
    def on_key_press(self, key):
        ...
        if self.__widget_img_view.reader is not None:
            ...
            if key in self.VAL_KEYS:
                ...
                self.__widget_img_view.set_frame(val=self.__widget_img_view.reader.frame_count - 1)
            elif key == Qt.Key_Comma:
                self.__widget_img_view.set_frame(val=0)
            ...
            if os.path.exists(source_root):
                for path in path_lst:
                    ...
                    if path.endswith('.mp4') and os.path.isfile(path):
                        self.__combo_sl.addItem(path)

    @pyqtSlot(int)
    def combo_sl_changed(self, selected_index):
        selected_video_path = self.__combo_sl.itemText(selected_index)
